...
async def submit_attendance(
    ...
    stmt = select(Attendance).where(Attendance.room_id == room_id, Attendance.user_id == current_user.id)
...
async def export_attendance(
    ...
    if room.host_id != current_user.id and current_user.role != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Hanya Host atau Admin yang dapat mengekspor absen")
...
async def sync_attendance_to_drive(
    ...
    if room.host_id != current_user.id and current_user.role != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Hanya Host atau Admin yang dapat mensinkronisasi absen")
    ...
    if "error" in sync_res:
        return Response(
            content=f"Gagal sinkron ke Drive: {sync_res['error']}",
            status_code=500,
            headers={"HX-Trigger": '{"showToast": {"msg": "❌ Gagal sinkron ke Drive", "type": "error"}}'}
        )
        
    return Response(
        content="Success",
        headers={"HX-Trigger": '{"showToast": {"msg": "✅ Absensi berhasil disimpan ke BaknusDrive!", "type": "success"}}'}
    )
...
async def sync_active_attendance(
    ...
    if not active_emails:
        return Response(
            ...
            headers={"HX-Trigger": '{"showToast": {"msg": "⚠️ Tidak ada peserta aktif terdeteksi", "type": "warning"}}'}
        )

    # 2. Convert emails bytes to strings
    ...
    return Response(
        content=f"Synced {added_count} participants",
        headers={"HX-Trigger": f'{{"showToast": {{"msg": "✅ Berhasil merekap {added_count} peserta baru!", "type": "success"}}, "attendanceUpdate": {{}}}}'}
    )
